<script lang="ts">
    ...
    $effect(() => {
        if (isChecking) return; // Wait until memory is read

        const currentPath = $page.url.pathname;

        // PUBLIC ROUTES (Must allow /auth-sync so the login loop doesn't break!)
    ...
    let navLinks = $derived(() => {
        if (isAdminPath) return []; // Hide client links on admin views
        
        // Hide all middle nav links if they are a completely logged-out guest
                            ...
                            <div class="flex items-center space-x-4">
                                {#if currentRole}
                                    ...
                                    <button 
                                        ...
                                        class="bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold py-2 px-4 rounded-md transition-colors cursor-pointer border-0"
                                    >
                                        Logout
                                    </button>
                                {:else}
