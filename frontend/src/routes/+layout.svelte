        ...
        {:else}
            <div class="min-h-screen bg-slate-50 flex flex-col font-sans">
                {#if !$page.url.pathname.startsWith("/login") && !$page.url.pathname.startsWith("/signup") && !$page.url.pathname.startsWith("/driver-apply") && !$page.url.pathname.startsWith("/auth-sync")}
                    <header
                        class="bg-slate-900 text-white shadow-md sticky top-0 z-50"
                    >
                        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                            <div class="flex items-center justify-between h-16">
                                <div class="flex items-center space-x-4">
                                    <span
                                        class="font-bold text-xl tracking-wider text-blue-400"
                                    >
                                        SHUTUP FORWARDER
                                    </span>
                                </div>
                                ...
                                <div class="flex items-center space-x-4">
                                    {#if currentRole}
                                        <span
                                            class="text-xs uppercase tracking-widest text-slate-400 bg-slate-800 px-2 py-1 rounded-md"
                                        >
                                            {displayRole()}
                                        </span>
